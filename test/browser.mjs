...
await page.addInitScript(() => {
  ...
  const record={id:8,record_code:'ARC-SCIENCE-000008',domain_id:'SCIENCE',category_id:'SPACE',title:{ko:'케플러-442b 발견',en:'Kepler-442b discovery',ja:'ケプラー442bの発見'},summary:{ko:'관측으로 확인된 외계행성 기록이다.',en:'An observed exoplanet record.'},content:{ko:'관측된 사실과 출처를 바탕으로 보존된 기록이다.',en:'A sourced observation record.'},event_date:'2015-01-06',tags:['외계행성','천문학'],source:'https://science.nasa.gov/exoplanet-catalog/kepler-442-b/',level:3,author_id:'00000000-0000-0000-0000-000000000000',created_at:'2026-01-01',keeper_code:'KEEPER-000',domain_name:domain.name,category_name:category.name,status:'published',content_available:true};
  class Query {
    ...
    result(){let data=[];if(this.table==='profiles')data=profile;else if(this.table==='domains')data=[domain];else if(this.table==='categories')data=[category];else if(this.table==='archive_statistics')data={record_count:30,today_count:1,keeper_count:7};else if(this.table==='record_catalog')data=[{...record,content:undefined}];else if(this.table==='records')data=this.one?record:[record];else if(this.table==='moderation_cases')data=[{id:12,record_id:8,report_count:3,status:'open',decision:null,opened_at:'2026-01-01',records:record,moderation_votes:[]}];return {data,error:null};}
    then(resolve,reject){return Promise.resolve(this.result()).then(resolve,reject);}
  }
  window.__mockClient={
    auth:{getSession:async()=>({data:{session:location.search.includes('guest=1')?null:{user}}}),getUser:async()=>({data:{user}}),onAuthStateChange:()=>({data:{subscription:{unsubscribe(){}}}}),signInWithPassword:async()=>({error:null}),signUp:async()=>({error:null}),resetPasswordForEmail:async()=>({error:null}),updateUser:async()=>({error:null}),signOut:async()=>({error:null})},
    from:table=>new Query(table),
    rpc:async(name)=>({data:name==='get_record_for_reader'?[record]:name==='search_record_catalog'?[record]:name==='get_related_records'?[]:[],error:null})
  };
});
