...
| `suite-ui.js` | 24 | 페이지네이션 · 언어 탭 · 폼 상한 · 신고 UI · 세션 만료 · 드로어 · 등급 상승 배너 · 관리자 대기열 · 회귀 2건 |
...
| `suite-hardening.js` | 13 | 저장된 마크업의 실행 여부 · 길이 경계 · 긴 문자열 레이아웃 · 애니메이션 규격 · 포커스 표시 · 키보드 이동 |
| `suite-session.js` | 9 | 토큰 자동 갱신과 갱신 실패 · 다중 탭 동기화 · 로그아웃 이후 · i18n 키 폴백 |
| `suite-race.js` | 9 | 느린 응답에서의 스켈레톤 · 화면 경합 · 이중 제출 · 두 탭 동시 수정 |
