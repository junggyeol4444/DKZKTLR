(function () {
  ...
  const sortRecords = records => { const sort=localStorage.getItem('akashic_sort')||'created',lang=localStorage.getItem('akashic_lang')||'ko',text=v=>v?.[lang]||v?.ko||v?.en||''; return records.sort((a,b)=>sort==='eventAsc'?String(a.event_date||'9999').localeCompare(String(b.event_date||'9999')):sort==='eventDesc'?String(b.event_date||'').localeCompare(String(a.event_date||'')):sort==='title'?text(a.title).localeCompare(text(b.title),lang):sort==='levelDesc'?b.level-a.level:String(b.created_at||'').localeCompare(String(a.created_at||''))); };
  ...
  window.ArchiveAPI = {
    ...
    resetPassword(email) { return getClient().auth.resetPasswordForEmail(email, { redirectTo: `${location.origin}${location.pathname}` }); },
    ...
    async moderationCases() { const { data, error } = await getClient().rpc('get_moderation_cases'); if(error) throw error; return data||[]; },
