(function () {
  ...
  window.ArchiveAPI = {
    ...
    resetPassword(email) { return getClient().auth.resetPasswordForEmail(email, { redirectTo: `${location.origin}${location.pathname}#/reset/update` }); },
    ...
    async records(filters = {}, page = 0) {
      ...
      if (filters.search) q = q.textSearch('search_document', filters.search, { type: 'websearch', config: 'simple' });
    ...
    async myRecords(userId) { const { data, error } = await getClient().from('records').select('*,profiles!records_author_id_fkey(keeper_code)').eq('author_id', userId).order('created_at', { ascending: false }); if (error) throw error; return data; },
    async ownRecord(code, userId) { const { data, error } = await getClient().from('records').select('*').eq('record_code',code).eq('author_id',userId).single(); if(error) throw error; return data; },
    ...
    async view(userId, recordId) { const { error } = await getClient().from('record_views').upsert({ user_id:userId, record_id:recordId, viewed_at:new Date().toISOString() }); if (error) throw error; },
    ...
    async moderationCases() { const { data, error } = await getClient().from('moderation_cases').select('id,record_id,report_count,status,decision,opened_at,resolved_at,records(record_code,title,summary,source,author_id,status),moderation_votes(admin_id,decision,note,created_at,profiles(keeper_code,display_name))').order('opened_at',{ascending:false}); if (error) throw error; if (!data.length) return data; const reports=await getClient().from('reports').select('record_id,reason,detail,created_at').in('record_id',[...new Set(data.map(x=>x.record_id))]); if(reports.error) throw reports.error; return data.map(x=>({...x,reports:reports.data.filter(r=>r.record_id===x.record_id)})); },
